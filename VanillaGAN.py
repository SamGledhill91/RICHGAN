...
with open('rand5_normalized.pkl', 'rb') as file:
...
for epoch in range(num_epochs):
    ...
    #generate samples
    generated_samples = generator(noise_vector).to(device)
    ...
    all_samples = torch.cat((train_tensor, generated_samples)).to(device)
    ...
    generator.zero_grad()
    noise_vector = torch.randn(train_size, noise_size).to(device)
    print(noise_vector.size())
    latent_space_samples = noise_vector
    #generate samples    
    ...
    if epoch % 5 == 1 and epoch !=1:
        ...
        points1 = np.array(train_tensor[500:510])
        xs1 = points1[:,:5].T
        ys1 = points1[:,5:].T
        points2 = np.array(points[500:510])
        xs2 = points2[:,:5].T
        ys2 = points2[:,5:].T
